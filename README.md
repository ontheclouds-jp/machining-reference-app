...
生産技術者・機械設計者・機械加工オペレーター向けの寸法早見PWAアプリ。ザグリ寸法・タップ下穴・はめあい公差の3タブを1画面に集約し、スマホでサイズを選ぶだけで参考値を表示します。
...
- `src/components` — タブ（ザグリ／タップ下穴／はめあい公差）ごとのUI
- `src/lib/data` — 仕様書3章のデータ定数
- `src/lib/tolerance.ts` — はめあい公差の計算ロジック
